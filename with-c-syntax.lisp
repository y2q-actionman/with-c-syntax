...
(defun finalize-struct-spec (sspec)
  ;; fills name
  ...
  (loop for (spec-qual . struct-decls)
     ...
     if (decl-specs-lisp-code spec-qual)
     ...
     append (loop with tp = (decl-specs-lisp-type spec-qual)
	       ...
	       as bits = (struct-declarator-bits s-decl)
               ...
               ;; Limit the bits. see above comments.
               do (error "invalid bitfield: ~A, ~A" tp s-decl)
     ...
     into fields
       ...
       (setf (struct-or-union-spec-lisp-code sspec)
...
(defun finalize-enum-spec (espec)
  ;; fills name
...
(defun lispify-type-spec (tp-list)
  (loop with numeric-type = nil 
     ...
     do (unless (= 1 (length tp-list))
	  (error "invalid decl-spec (~A)" tp-list))
       (return (finalize-struct-spec tp))

     else if (enum-spec-p tp)	; enum
     do (unless (= 1 (length tp-list))
	  (error "invalid decl-spec (~A)" tp-list))
       (return (finalize-enum-spec tp))
...
(defun finalize-decl-specs (dspecs)
  (let ((ltype (lispify-type-spec (decl-specs-type-spec dspecs))))
    (cond ((enum-spec-p ltype)
	   (setf (decl-specs-lisp-type dspecs)
		 (enum-spec-lisp-type ltype))
	   ;; TODO: move from here??
