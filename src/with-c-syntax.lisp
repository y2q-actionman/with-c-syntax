...
;;; Toplevel
(declaim (inline unwrap-const-closure))
(defun unwrap-const-closure (x)
  (declare (optimize (speed 3) (safety 0) (debug 0))
	   (type (function () t) x))
  (funcall x))

(declaim (inline (setf unwrap-const-closure)))
(defun (setf unwrap-const-closure) (val x)
  (error 'runtime-error :format-control "assigning ~A to const ~A"
	 :format-arguments (list val x)))
...
(defun expand-toplevel-init-decls (mode dspecs init-decls
  ...
  (loop with storage-class = (or (decl-specs-storage-class dspecs)
     ...
     with const-qualified =  nil ;;(member '|const| qualifiers)
     ...
     ;; variables
       ...
       (ecase storage-class
         ((|auto| |register|)		; 'auto' or 'register' vars
          (when (eq mode :translation-unit)
            (error 'compile-error
                   :format-control "At top level, '~A' variables are not accepted (~S)."
                   :format-arguments (list storage-class name)))
	  ;; 'const' processing
	  (if const-qualified
	      (let ((const-holder (gensym "auto-const-")))
		;; See: https://g000001.cddddr.org/3703854697
		;; 
		;; FIXME: this code will fail if the next lexical variable use this value.
		;; I must chain `symbol-macrolet'!
		(push `(,const-holder (lambda () ,init)) lexical-binds)
		(push `(,name (unwrap-const-closure ,const-holder)) sym-macro-defs)
		(push const-holder dynamic-extent-vars))
	      (push `(,name ,init) lexical-binds))
	  ;; 'register' only code
	  (when (eq storage-class '|register|)
         ...
         (|global|			; 'global' vars.
          (when (eq mode :statement)
            (error 'compile-error
                   ...
                   :format-arguments (list name)))
	  (let ((def-param-op (if const-qualified 'defconstant 'defparameter)))
	    (if (constantp init *wcs-expanding-environment*)
		(push `(,def-param-op ,name ,init
		...
		(let ((init-sym (gensym (format nil "global-var-~A-tmp-" name))))
		  ...
		  (push `(,def-param-op ,name ,init-sym
	  ...
	  (ecase mode
	    (:statement
	     ...
	     (let ((def-var-op (if const-qualified 'defconstant 'defvar))
		   (st-sym (gensym (format nil "static-var-~A-storage-" name))))
	       (if (constantp init *wcs-expanding-environment*)
		   (push `(,def-var-op ,st-sym ,init
		   ...
		   (let ((st-init-sym (gensym (format nil "static-var-~A-tmp-" name))))
		     ...
		     (push `(,def-var-op ,st-sym ,st-init-sym
