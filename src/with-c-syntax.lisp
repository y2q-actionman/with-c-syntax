  ...
  (loop with storage-class = (or storage-class-in-decl-specs
     ...
     ;; variables
	  ...
	  (let ((init-sym (gensym (format nil "global-var-~A-tmp-" name))))
	    ...
	    ;; (push name special-vars) ; commented out, I thought this is not needed.
	  ...
	  (ecase mode
	    (:statement
	     ...
	     (let ((st-sym (gensym (format nil "static-var-~A-storage-" name)))
		   (st-init-sym (gensym (format nil "static-var-~A-tmp-" name))))
	       (push `(,st-init-sym ,init) lexical-binds)
	       (push `(,name ,st-sym) sym-macro-defs)
	       ;; (push st-sym special-vars) ; commented out, I thought this is not needed.
